...
export function parseOcrCardData(extractedData) {
  ...
  // AI บางครั้งอ่านป้ายกำกับ "ชื่อ-สกุล" เพี้ยน (เช่น "ซีอ-สกุล") ทำให้ตัดคำไม่ออกแล้วส่วนที่เหลือ
  // ("ซีอ-") ติดเข้ามาเป็นส่วนหนึ่งของชื่อ — ถ้าคำแรกมีขีดกลางและคำถัดไปคือคำนำหน้าชื่อจริงๆ
  // ให้ทิ้งคำแรกนั้น (ชื่อคนไทยไม่มีขีดกลาง จึงไม่กระทบชื่อปกติ)
  rawName = rawName.replace(/^\S*-\S*\s+(?=(นางสาว|นาย|นาง))/, '');
